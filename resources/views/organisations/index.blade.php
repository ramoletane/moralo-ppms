...
{{--                            @if ($organisation->user->is(auth()->user())) --}}
                                <x-dropdown>
                                    <x-slot name="trigger">
                                        <button>
                                    ...
                                    <x-slot name="content">
                                        ...
                                        <form method="POST" action="{{ route('organisations.destroy', $organisation) }}">
                                            @csrf
                                            @method('delete')
                                            <x-dropdown-link :href="route('organisations.destroy', $organisation)" onclick="event.preventDefault(); this.closest('form').submit();">
                                                {{ __('Delete') }}
                                            </x-dropdown-link>
                                        </form>
                                    </x-slot>
...
{{--                            @endif --}}
        ...
        <x-primary-button-link :href="route('organisations.create')" class="mt-4">{{ __('Add Organisation') }}</x-primary-button>
